...
function(_pulse_sdk_find_first_target out_var)
...
function(_pulse_sdk_make_alias alias_name target_name)
	if(NOT TARGET ${alias_name})
		add_library(${alias_name} ALIAS ${target_name})
	endif()
endfunction()

function(_pulse_sdk_add_cmake_dep root dep_name out_target)
	...
	set(_binary_dir "${CMAKE_BINARY_DIR}/pulse-sdk-deps/${dep_name}-${_root_id}")
...
function(_pulse_sdk_resolve_metrics_user_defines out_var)
	set(_metrics_defines "")

	if(DEFINED PULSE_SDK_METRICS_USER_DEFINES AND
			NOT PULSE_SDK_METRICS_USER_DEFINES STREQUAL "")
		set(_metrics_defines "${PULSE_SDK_METRICS_USER_DEFINES}")
...
function(_pulse_sdk_resolve_cmake_dependency dep_name preferred_var fallback_var
		local_dir repo_url repo_tag alias_name out_target out_managed out_source)
	_pulse_sdk_find_first_target(_existing_target ${alias_name} ${dep_name})
	if(_existing_target)
		if(NOT _existing_target STREQUAL ${alias_name})
			_pulse_sdk_make_alias(${alias_name} ${_existing_target})
		endif()
	...
	elseif(EXISTS "${PULSE_SDK_ROOT}/${local_dir}")
		set(_resolved_root "${PULSE_SDK_ROOT}/${local_dir}")
	...
	elseif(PULSE_SDK_FETCH_DEPS)
		...
		_pulse_sdk_make_alias(${alias_name} ${dep_name})
		set(${out_target} ${alias_name} PARENT_SCOPE)
		set(${out_managed} TRUE PARENT_SCOPE)
		set(${out_source} fetched PARENT_SCOPE)
		return()
	elseif(EXISTS "${PULSE_SDK_ROOT}/../${dep_name}")
		set(_resolved_root "${PULSE_SDK_ROOT}/../${dep_name}")
		set(_resolved_source sibling)
	endif()

	if(NOT _resolved_root)
		message(FATAL_ERROR
			"${dep_name} root not found. Set ${preferred_var} or ${fallback_var}, place dependency under ${local_dir}, or enable PULSE_SDK_FETCH_DEPS.")
	endif()

	_pulse_sdk_add_cmake_dep("${_resolved_root}" ${dep_name} _resolved_target)
	_pulse_sdk_make_alias(${alias_name} ${_resolved_target})
...
function(_pulse_sdk_get_platform_name out_var)
	set(_platform "")
...
function(_pulse_sdk_append_transport_compile_definition out_var)
	set(_defs ${ARGN})
	_pulse_sdk_get_transport_name(_transport)
	if(_transport STREQUAL "coaps")
		list(APPEND _defs PULSE_SDK_TRANSPORT_COAPS=1)
	else()
		list(APPEND _defs PULSE_SDK_TRANSPORT_COAPS=0)
	endif()

	set(${out_var} ${_defs} PARENT_SCOPE)
endfunction()

function(_pulse_sdk_append_linux_transport_links srcs_var links_var)
...
function(_pulse_sdk_is_idf_component_available dep_name out_var)
...
function(pulse_sdk_collect_cmake out_srcs out_public_incs out_private_incs)
	set(_srcs
	...
	_pulse_sdk_resolve_cmake_dependency(libmcu PULSE_SDK_LIBMCU_ROOT LIBMCU_ROOT
		external/libmcu https://github.com/libmcu/libmcu.git main
		pulse::dep::libmcu _pulse_sdk_libmcu_target
		_pulse_sdk_libmcu_managed _pulse_sdk_libmcu_source)
	_pulse_sdk_resolve_cmake_dependency(cbor PULSE_SDK_CBOR_ROOT CBOR_ROOT
		external/cbor https://github.com/libmcu/cbor.git main
		pulse::dep::cbor _pulse_sdk_cbor_target
		_pulse_sdk_cbor_managed _pulse_sdk_cbor_source)

	_pulse_sdk_resolve_metrics_user_defines(_metrics_user_defines)
	if(_pulse_sdk_libmcu_managed AND _metrics_user_defines)
		target_compile_definitions(libmcu PUBLIC
			METRICS_USER_DEFINES="${_metrics_user_defines}")
	endif()

	list(APPEND _public_links ${_pulse_sdk_libmcu_target})
	list(APPEND _private_links ${_pulse_sdk_cbor_target})

	_pulse_sdk_append_transport_compile_definition(_public_defs ${_public_defs})
	_pulse_sdk_get_platform_name(_platform)
	...
	_pulse_sdk_append_linux_transport_links(_srcs _public_links)
...
function(pulse_sdk_collect out_srcs out_public_incs out_private_incs)
	...
	if(_platform STREQUAL "esp-idf")
		_pulse_sdk_is_idf_component_available(libmcu _reuse_libmcu)
		_pulse_sdk_is_idf_component_available(cbor _reuse_cbor)
	...
	# NOTE: pulse-sdk always compiles its own transport/override sources so the
	...
	if(NOT _reuse_libmcu AND
			(_PULSE_SDK_LIBMCU_RESOLVED_ROOT_SOURCE STREQUAL "bundled" OR
